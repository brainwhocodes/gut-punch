...
export function createSchema(prefix: string = '') {
  ...
  const jobRuns = sqliteTable(tableName("job_runs"), {
  ...
  const jobs = sqliteTable(tableName("jobs"), {
  ...
  const scheduledJobs = sqliteTable(tableName("scheduled_jobs"), {
    job_name: text("job_name").primaryKey(),
    next_run: text("next_run").notNull(),
  });

  // Return the schema object
  return { jobRuns, jobs, scheduledJobs };
}
